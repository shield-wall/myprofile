...
export class Section implements OptionalTitleInterface, OptionalIconInterface {
	...
	constructor(sectionType: SectionType) {
		...
		this.children = sectionType.children.map(
			(
				item:
					| SimpleListType
					| EducationType
				...
				if ("degree" in item) return new Education(item);

				if ("company" in item) return new Experience(item);

				if ("tag" in item) return new Tag(item);

				if ("content" in item) return new Content(item);

				if ("photoUrl" in item) return new Certification(item);
